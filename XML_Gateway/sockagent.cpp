...
void CSockagentApp::sockReceive(sock* client)
{
	CString string;
	char ch[2048];
	for (int i=0;i<2048;i++) ch[i]=0x00;
	int status;

	if((status=client->Receive(ch,2048,0))>0)	//TCP
	{
		string.Format(ch);
		OutputString(string);
	...
	string.Empty();
//test ack
	if(CString(ch).CompareNoCase("hi albert")!=0)
		client->SendMsg(CString("<GWPOST><Result>Fail</Result></GWPOST>"));
...
int CSockagentApp::ClientHandler(char *byte)
{
	byte[0]='w';
	return 1;
}
